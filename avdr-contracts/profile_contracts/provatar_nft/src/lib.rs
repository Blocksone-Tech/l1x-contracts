...
const STORAGE_CIDS_KEY: &[u8] = b"cids";
...
impl NftContract {
    pub fn new(metadata: NFTMetadata) {
        ...
        let mut contract = Self {
            ...
            cids: LookupMap::new(STORAGE_CIDS_KEY.to_vec()),
    ...
    pub fn nft_mint_to(to: Address, cid: Cid) -> U128 {
        // load the contract storage state
        let mut contract = Self::load();

        // Call the internal implementation
        let new_token_id = contract.mint_to(to, cid);
    ...
    pub fn nft_mint_id_to(to: Address, id: U128, cid: Cid) -> U128 {
        // load the contract storage state
        let mut contract = Self::load();

        // Call the internal implementation
        let new_token_id = contract.mint_id_to(to, id.into(), cid);
    ...
    pub fn nft_token_cid(id: U128) -> Cid {
        let contract = Self::load();
        contract.token_cid(id.into())
    }
}
...
impl NftContract {
    ...
    fn mint_id_to(&mut self, to: Address, id: u128, cid: Cid) -> u128 {
        let new_token_id = id;
    ...
    fn mint_to(&mut self, to: Address, cid: Cid) -> u128 {
        ...
        self.mint_id_to(to, new_token_id, cid);

        new_token_id
    }

    fn burn(&mut self, id: u128) {
        let owner_info = self.owner_of.get(&id);
        assert!(owner_info.is_some(), "Token ID {:#?} Not Minted or Doesn't exist", id);

        assert_eq!(owner_info.unwrap().address, caller_address(), "The caller doesn't own Token ID");
    ...
    fn token_cid(&self, id: u128) -> Cid {
        self.cids.get(&id).expect(&format!("TokenId: {:#?} is not minted or doesn't exist in the contract", id)).clone()
    }
