...
use l1x_sdk::{
    caller_address, contract, contract_instance_address, contract_owner_address, emit_event_experimental,
    store::{LookupMap, Vector},
    types::{Address, U128},
};
use serde::Deserialize;
use std::collections::{BTreeMap, BTreeSet};
use types::name_service_interface::NameServiceContract;
...
const STORAGE_TOKEN_NAMES_KEY: &[u8] = b"token-names";

/// Token Total Supply Configuration
const L1X_NFT_TOTAL_SUPPLY: u128 = 10_0000;
...
pub struct NftContract {
    ...
    name_service: NameServiceContract,
    token_names: LookupMap<u128, String>,
    burned_nfts: BTreeSet<u128>,
}

#[contract]
impl NftContract {
    pub fn new(metadata: NFTMetadata, name_service_address: Address) {
        Self::assert_owner();
        ...
        let mut contract = Self {
            ...
            name_service: NameServiceContract::new(name_service_address),
            token_names: LookupMap::new(STORAGE_TOKEN_NAMES_KEY.to_vec()),
    ...
    pub fn nft_mint_to(to: Address, name: String) -> U128 {
        ...
        let new_token_id = contract.mint_to(to, name);
    ...
    pub fn nft_mint_id_to(to: Address, id: U128, name: String) -> U128 {
        assert!(name.len() <= MAX_NAME_LENGTH, "name's length should be less than 20");
        // load the contract storage state
        let mut contract = Self::load();

        // Call the internal implementation
        let new_token_id = contract.mint_id_to(to, id.into(), name);
    ...
    pub fn nft_token_name(id: U128) -> String {
        let contract = Self::load();
        contract.token_name(id.into())
    }
}
...
impl NftContract {
    ...
    fn internal_remove_token(&mut self, id: u128) -> (Address, u32) {
        // Update the balances
        let owner_info = self.owner_of.get(&id).cloned().unwrap_or_else(|| panic!("No owner with id {}", id));

        let balance_from = self.balance_of.get_mut(&owner_info.address).expect("Not enough funds to burn or transfer");
        ...
        if !is_last {
            ...
            let swapped_token_id = balance_from.get(owner_info.token_idx).expect("Can't get the swapped token_id");
            let owner_ref =
                self.owner_of.get_mut(swapped_token_id).expect("Can't find an owner of the swapped token_id");
    ...
    fn internal_add_token_to(&mut self, to: Address, id: u128) {
        ...
        let balance_to = if let Some(v) = self.balance_of.get_mut(&to) {
            ...
            self.balance_of.get_mut(&to).expect("Can't get the just added Vector")
        };
    ...
    fn mint_id_to(&mut self, to: Address, id: u128, name: String) -> u128 {
        let new_token_id = id;
        assert!(!self.burned_nfts.contains(&id), "Burned Token ID {:?} cannot be minted again", new_token_id);
        assert!(!self.owner_of.contains_key(&new_token_id), "Token ID {:?} already exist", new_token_id);
        assert!(new_token_id <= L1X_NFT_TOTAL_SUPPLY, "Max supply reached");

        self.register_or_transfer_name(to, name.clone());
        self.internal_add_token_to(to, new_token_id);
        self.token_names.insert(new_token_id, name);

        self.minted_total += 1;

        // Emit the Token minted event
        emit_event_experimental(NftEvent::NftTokenMinted(format!("Minted token {:#?} for owner {}", new_token_id, to)));

        l1x_sdk::msg(&format!("Minted token {:#?} for owner {}", new_token_id, to));

        new_token_id
    }

    fn mint_to(&mut self, to: Address, name: String) -> u128 {
        ...
        self.mint_id_to(to, new_token_id, name);

        new_token_id
    }

    fn burn(&mut self, id: u128) {
        let token_owner = self.owner_of.get(&id);
        let caller = caller_address();

        assert!(self.owner_of.get(&id).is_some(), "Token ID {:#?} Not Minted or Doesn't exist", id);
        assert_eq!(token_owner.unwrap().address, caller, "The caller doesn't own Token ID {:#?}", id);

        let name = self.token_names.remove(id).expect("Token ID Not Minted or Doesn't exist").clone();
        let (from, balance_from) = self.internal_remove_token(id);
        self.name_service.transfer_name(name, from);
        ...
        l1x_sdk::msg(&format!("Burn Token_ID {:#?} from Owner {} Balance {:#?}", id, from, balance_from));
    }
    ...
    fn approve(&mut self, spender: Address, id: u128) {
        ...
        assert!(self.owner_of.get(&id).is_some(), "TokenId: {:#?} is not minted or doesn't exist in the contract", &id,);
    ...
    fn set_approval_for_all(&mut self, operator: Address, approved: bool) {
        ...
        if let Some(approved_map) = self.is_approved_for_all.get_mut(&caller_id) {
            ...
            self.is_approved_for_all.insert(caller_id.clone(), new_approved_map);
        }
    ...
    fn transfer_from(&mut self, from: Address, to: Address, id: u128) {
        ...
        let name = self.token_name(id);

        self.name_service.change_name_resolver(name, to);

        // Emit transfer done event
        emit_event_experimental(NftEvent::NftTokenTransferred(format!(
            "Token Transfer done for Token_id {:#?} From {} To {}",
            id, from, to
        )));

        l1x_sdk::msg(&format!("Token Transfer done for Token_id {:#?} From {} To {}", id, from, to));
    }
    ...
    fn owner_of(&self, id: u128) -> Address {
        ...
        // Check if the ID exists in the contract's owner_of mapping or assign default
        assert!(owner.is_some(), "TokenId: {:#?} is not minted or doesn't exist in the contract", &id,);

        owner.unwrap().address.clone()
    }

    fn owned_tokens(&self, owner: Address) -> Vec<U128> {
        let issued_tokens = self.balance_of.get(&owner).unwrap_or_else(|| panic!("Not enough funds"));
    ...
    fn register_or_transfer_name(&mut self, user: Address, name: String) {
        let caller_owns_names = self.name_service.list_owned_names(user);
        // If the caller owns the name:
        // 1. Transfer the name to this instance address
        // 2. Change the resolver address to the caller
        if caller_owns_names.iter().find(|n| n.name == name).is_some() {
            self.name_service.transfer_name(name.clone(), contract_instance_address());
            self.name_service.change_name_resolver(name, user);
        } else {
            // Otherwise register a new name
            self.name_service.register_root_name(name, Some(user));
        }
    }

    fn token_name(&self, id: u128) -> String {
        self.token_names.get(&id).expect("Can't find the Token Name").clone()
    }
